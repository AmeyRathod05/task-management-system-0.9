"use client";
import React, { useState, useRef, useEffect } from "react";
import { Task } from "./types/Task";
import Image from "next/image";

interface EditableTaskItemProps extends Task {
  onDragStart: (e: React.DragEvent<HTMLDivElement>) => void;
  onTaskUpdate: (taskId: string, updates: Partial<Task>) => void;
  projects: Array<{ id: string; name: string }>;
}

const EditableTaskItem: React.FC<EditableTaskItemProps> = ({
  id,
  name,
  description,
  isChecked,
  dueDate,
  startDate,
  endDate,
  commentCount,
  category,
  userAvatar,
  status,
  priority,
  projectId,
  projectName,
  assignedTo,
  onDragStart,
  toggleChecked,
  onTaskUpdate,
  projects,
}) => {
  const [isEditing, setIsEditing] = useState<string | null>(null);
  const [editValues, setEditValues] = useState({
    name,
    description: description || "",
    dueDate,
    startDate: startDate || "",
    endDate: endDate || "",
    priority,
    projectId: projectId || "",
  });

  const inputRef = useRef<HTMLInputElement>(null);
  const textareaRef = useRef<HTMLTextAreaElement>(null);

  useEffect(() => {
    if (isEditing === "name" && inputRef.current) {
      inputRef.current.focus();
      inputRef.current.select();
    }
    if (isEditing === "description" && textareaRef.current) {
      textareaRef.current.focus();
    }
  }, [isEditing]);

  const handleEdit = (field: string) => {
    setIsEditing(field);
  };

  const handleSave = (field: string) => {
    const updates: Partial<Task> = {};
    
    switch (field) {
      case "name":
        updates.name = editValues.name;
        break;
      case "description":
        updates.description = editValues.description;
        break;
      case "dueDate":
        updates.dueDate = editValues.dueDate;
        break;
      case "startDate":
        updates.startDate = editValues.startDate;
        break;
      case "endDate":
        updates.endDate = editValues.endDate;
        break;
      case "priority":
        updates.priority = editValues.priority;
        break;
      case "projectId":
        updates.projectId = editValues.projectId;
        const selectedProject = projects.find(p => p.id === editValues.projectId);
        updates.projectName = selectedProject?.name;
        break;
    }

    onTaskUpdate(id, updates);
    setIsEditing(null);
  };

  const handleCancel = () => {
    setEditValues({
      name,
      description: description || "",
      dueDate,
      startDate: startDate || "",
      endDate: endDate || "",
      priority,
      projectId: projectId || "",
    });
    setIsEditing(null);
  };

  const handleKeyPress = (e: React.KeyboardEvent, field: string) => {
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      handleSave(field);
    }
    if (e.key === "Escape") {
      handleCancel();
    }
  };

  const getPriorityColor = (priority: number) => {
    switch (priority) {
      case 0: return "bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400";
      case 1: return "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400";
      case 2: return "bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400";
      case 3: return "bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400";
      default: return "bg-gray-100 text-gray-800 dark:bg-gray-900/20 dark:text-gray-400";
    }
  };

  const getPriorityLabel = (priority: number) => {
    switch (priority) {
      case 0: return "Low";
      case 1: return "Medium";
      case 2: return "High";
      case 3: return "Critical";
      default: return "Low";
    }
  };

  return (
    <div
      id={`task-${id}`}
      draggable="true"
      onDragStart={onDragStart}
      className="p-5 mb-4 bg-white border border-gray-200 task rounded-xl shadow-theme-sm dark:border-gray-800 dark:bg-white/5 hover:shadow-md transition-shadow"
    >
      <div className="space-y-4">
        {/* Header with checkbox and drag handle */}
        <div className="flex items-start gap-4">
          <span className="text-gray-400 cursor-move">
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M2.43311 5.0001C2.43311 4.50304 2.83605 4.1001 3.33311 4.1001L16.6664 4.1001C17.1635 4.1001 17.5664 4.50304 17.5664 5.0001C17.5664 5.49715 17.1635 5.9001 16.6664 5.9001L3.33311 5.9001C2.83605 5.9001 2.43311 5.49716 2.43311 5.0001ZM2.43311 15.0001C2.43311 14.503 2.83605 14.1001 3.33311 14.1001L16.6664 14.1001C17.1635 14.1001 17.5664 14.503 17.5664 15.0001C17.5664 15.4972 17.1635 15.9001 16.6664 15.9001L3.33311 15.9001C2.83605 15.9001 2.43311 15.4972 2.43311 15.0001ZM3.33311 9.1001C2.83605 9.1001 2.43311 9.50304 2.43311 10.0001C2.43311 10.4972 2.83605 10.9001 3.33311 10.9001L16.6664 10.9001C17.1635 10.9001 17.5664 10.4972 17.5664 10.0001C17.5664 9.50304 17.1635 9.1001 16.6664 9.1001L3.33311 9.1001Z"
                fill="currentColor"
              />
            </svg>
          </span>

          <div className="flex items-center">
            <input
              type="checkbox"
              id={`taskCheckbox${id}`}
              className="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700"
              checked={isChecked}
              onChange={toggleChecked}
            />
          </div>

          <div className="flex-1">
            {/* Task Name */}
            {isEditing === "name" ? (
              <div className="flex items-center gap-2">
                <input
                  ref={inputRef}
                  type="text"
                  value={editValues.name}
                  onChange={(e) => setEditValues({ ...editValues, name: e.target.value })}
                  onKeyDown={(e) => handleKeyPress(e, "name")}
                  className="flex-1 px-2 py-1 text-base font-medium border border-blue-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                />
                <button
                  onClick={() => handleSave("name")}
                  className="p-1 text-green-600 hover:text-green-700"
                >
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                    <path d="M13.5 4.5L6 12L2.5 8.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </button>
                <button
                  onClick={handleCancel}
                  className="p-1 text-red-600 hover:text-red-700"
                >
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                    <path d="M12 4L4 12M4 4L12 12" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                  </svg>
                </button>
              </div>
            ) : (
              <h3
                onClick={() => handleEdit("name")}
                className="text-base font-medium text-gray-800 dark:text-white cursor-pointer hover:text-blue-600 dark:hover:text-blue-400"
              >
                {name}
              </h3>
            )}

            {/* Description */}
            {isEditing === "description" ? (
              <div className="mt-2">
                <textarea
                  ref={textareaRef}
                  value={editValues.description}
                  onChange={(e) => setEditValues({ ...editValues, description: e.target.value })}
                  onKeyDown={(e) => handleKeyPress(e, "description")}
                  placeholder="Add description..."
                  className="w-full px-2 py-1 text-sm border border-blue-300 rounded resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                  rows={3}
                />
                <div className="flex gap-2 mt-1">
                  <button
                    onClick={() => handleSave("description")}
                    className="px-2 py-1 text-xs text-white bg-green-600 rounded hover:bg-green-700"
                  >
                    Save
                  </button>
                  <button
                    onClick={handleCancel}
                    className="px-2 py-1 text-xs text-white bg-red-600 rounded hover:bg-red-700"
                  >
                    Cancel
                  </button>
                </div>
              </div>
            ) : (
              <p
                onClick={() => handleEdit("description")}
                className="mt-1 text-sm text-gray-600 dark:text-gray-400 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400"
              >
                {description || "Click to add description..."}
              </p>
            )}
          </div>
        </div>

        {/* Project and Priority */}
        <div className="flex flex-wrap gap-2">
          {/* Project */}
          {isEditing === "projectId" ? (
            <div className="flex items-center gap-2">
              <select
                value={editValues.projectId}
                onChange={(e) => setEditValues({ ...editValues, projectId: e.target.value })}
                className="px-2 py-1 text-xs border border-blue-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
              >
                <option value="">No Project</option>
                {projects.map((project) => (
                  <option key={project.id} value={project.id}>
                    {project.name}
                  </option>
                ))}
              </select>
              <button
                onClick={() => handleSave("projectId")}
                className="p-1 text-green-600 hover:text-green-700"
              >
                <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
                  <path d="M13.5 4.5L6 12L2.5 8.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </button>
            </div>
          ) : (
            <span
              onClick={() => handleEdit("projectId")}
              className="inline-flex rounded-full bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700 cursor-pointer hover:bg-blue-100 dark:bg-blue-500/15 dark:text-blue-400"
            >
              {projectName || "No Project"}
            </span>
          )}

          {/* Priority */}
          {isEditing === "priority" ? (
            <div className="flex items-center gap-2">
              <select
                value={editValues.priority}
                onChange={(e) => setEditValues({ ...editValues, priority: parseInt(e.target.value) })}
                className="px-2 py-1 text-xs border border-blue-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
              >
                <option value={0}>Low</option>
                <option value={1}>Medium</option>
                <option value={2}>High</option>
                <option value={3}>Critical</option>
              </select>
              <button
                onClick={() => handleSave("priority")}
                className="p-1 text-green-600 hover:text-green-700"
              >
                <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
                  <path d="M13.5 4.5L6 12L2.5 8.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </button>
            </div>
          ) : (
            <span
              onClick={() => handleEdit("priority")}
              className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium cursor-pointer hover:opacity-80 ${getPriorityColor(priority)}`}
            >
              {getPriorityLabel(priority)}
            </span>
          )}
        </div>

        {/* Dates and Meta Info */}
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap gap-4">
            {/* Due Date */}
            {isEditing === "dueDate" ? (
              <div className="flex items-center gap-2">
                <input
                  type="date"
                  value={editValues.dueDate}
                  onChange={(e) => setEditValues({ ...editValues, dueDate: e.target.value })}
                  className="px-2 py-1 text-xs border border-blue-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                />
                <button
                  onClick={() => handleSave("dueDate")}
                  className="p-1 text-green-600 hover:text-green-700"
                >
                  <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
                    <path d="M13.5 4.5L6 12L2.5 8.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </button>
              </div>
            ) : (
              <span
                onClick={() => handleEdit("dueDate")}
                className="flex items-center gap-1 text-sm text-gray-500 cursor-pointer hover:text-blue-600 dark:text-gray-400"
              >
                <svg
                  className="fill-current"
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M5.33329 1.0835C5.74751 1.0835 6.08329 1.41928 6.08329 1.8335V2.25016L9.91663 2.25016V1.8335C9.91663 1.41928 10.2524 1.0835 10.6666 1.0835C11.0808 1.0835 11.4166 1.41928 11.4166 1.8335V2.25016L12.3333 2.25016C13.2998 2.25016 14.0833 3.03366 14.0833 4.00016V6.00016L14.0833 12.6668C14.0833 13.6333 13.2998 14.4168 12.3333 14.4168L3.66663 14.4168C2.70013 14.4168 1.91663 13.6333 1.91663 12.6668L1.91663 6.00016L1.91663 4.00016C1.91663 3.03366 2.70013 2.25016 3.66663 2.25016L4.58329 2.25016V1.8335C4.58329 1.41928 4.91908 1.0835 5.33329 1.0835ZM5.33329 3.75016L3.66663 3.75016C3.52855 3.75016 3.41663 3.86209 3.41663 4.00016V5.25016L12.5833 5.25016V4.00016C12.5833 3.86209 12.4714 3.75016 12.3333 3.75016L10.6666 3.75016L5.33329 3.75016ZM12.5833 6.75016L3.41663 6.75016L3.41663 12.6668C3.41663 12.8049 3.52855 12.9168 3.66663 12.9168L12.3333 12.9168C12.4714 12.9168 12.5833 12.8049 12.5833 12.6668L12.5833 6.75016Z"
                    fill=""
                  />
                </svg>
                {dueDate}
              </span>
            )}

            <span className="flex items-center gap-1 text-sm text-gray-500 cursor-pointer dark:text-gray-400">
              <svg
                width="18"
                height="18"
                viewBox="0 0 18 18"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M9 15.6343C12.6244 15.6343 15.5625 12.6961 15.5625 9.07178C15.5625 5.44741 12.6244 2.50928 9 2.50928C5.37563 2.50928 2.4375 5.44741 2.4375 9.07178C2.4375 10.884 3.17203 12.5246 4.35961 13.7122L2.4375 15.6343H9Z"
                  stroke="currentColor"
                  strokeWidth="1.5"
                  strokeLinejoin="round"
                />
              </svg>
              {commentCount}
            </span>
          </div>

          <div className="h-6 w-full max-w-6 overflow-hidden rounded-full border-[0.5px] border-gray-200 dark:border-gray-800">
            <Image width={24} height={24} src={userAvatar} alt="user" />
          </div>
        </div>
      </div>
    </div>
  );
};

export default EditableTaskItem;
